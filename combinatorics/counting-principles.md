...
- Consider $k$ finite sets $A1$, $A2$, ..., $An$ and $B$, where $k >= 1$ and **all** given sets are **pairwise disjoint**, i.e. $`A_i ∩ A_j = ∅`$ where $1 <= i,j <= k$ and $i ≠ j$.
...
- Consider $k$ finite sets $`A_1`$, $`A_2`$, ..., $`A_k`$ and $B$, where $k >= 1$ and **all** given sets are **pairwise disjoint**, i.e. $`A_i ∩ A_j = ∅`$ where $1 <= i,j <= k$ and $i ≠ j$.
